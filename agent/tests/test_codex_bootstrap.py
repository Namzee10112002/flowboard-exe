...
def test_build_cli_env_sets_codex_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("FLOWBOARD_CODEX_HOME", str(codex_home))
...
def test_build_cli_env_strips_ambient_openai_and_codex_env(monkeypatch):
    ...
    ignored_codex = codex_ignored_codex_env_names()
    assert "CODEX_CI" in ignored_codex
    assert "CODEX_INTERNAL_ORIGINATOR_OVERRIDE" in ignored_codex
